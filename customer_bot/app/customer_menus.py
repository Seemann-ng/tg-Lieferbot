...
cart_button = lambda lang_code: types.InlineKeyboardButton(
    text=texts[lang_code]["CART_BTN"],
    callback_data=texts[lang_code]["CART_BTN"]
)
cancel_order_button = lambda lang_code: types.InlineKeyboardButton(
    text=texts[lang_code]["CANCEL_ORDER_BTN"],
...
def reg_phone_menu(lang_code: str) -> types.ReplyKeyboardMarkup:
    ...
    """
    reg_phone_import_button = types.KeyboardButton(
...
def reg_location_menu(lang_code: str) -> types.ReplyKeyboardMarkup:
    ...
    """
    reg_location_button = types.KeyboardButton(
        text=texts[lang_code]["REG_LOCATION_BTN"],
...
def reset_info_menu(lang_code: str) -> types.ReplyKeyboardMarkup:
    ...
    """
    confirm_contact_info_reset_button = types.KeyboardButton(
        text=texts[lang_code]["CONFIRM_RESET_BTN"]
    )
    menu = types.ReplyKeyboardMarkup(
        resize_keyboard=True,
...
def confirm_delete_profile_menu(lang_code: str) -> types.ReplyKeyboardMarkup:
    ...
    """
    confirm_delete_button = types.KeyboardButton(
        text=texts[lang_code]["CONFIRM_DELETE_PROFILE_BTN"]
    )
    menu = types.ReplyKeyboardMarkup(
        resize_keyboard=True,
...
def confirm_location_menu(lang_code: str) -> types.InlineKeyboardMarkup:
    ...
    """
    conf_loc_button = types.InlineKeyboardButton(
        text=texts[lang_code]["CONFIRM_LOCATION_BTN"],
        callback_data=texts[lang_code]["CONFIRM_LOCATION_BTN"]
    )
    wrong_loc_button = types.InlineKeyboardButton(
        text=texts[lang_code]["WRONG_LOCATION_BTN"],
        ...
        for restaurant_type in restaurant_types:
            menu.add(
                types.InlineKeyboardButton(
                    text=restaurant_type[0],
                    callback_data=restaurant_type[0]
                )
            )
...
def cart_menu(lang_code: str) -> types.InlineKeyboardMarkup:
    ...
    pay_button = types.InlineKeyboardButton(
        text=texts[lang_code]["MAKE_ORDER_BTN"],
        callback_data=texts[lang_code]["MAKE_ORDER_BTN"]
    )
    add_comment_button = types.InlineKeyboardButton(
        text=texts[lang_code]["ADD_COMMENT_BTN"],
        callback_data=texts[lang_code]["ADD_COMMENT_BTN"]
    )
    add_more_button = types.InlineKeyboardButton(
        text=texts[lang_code]["ADD_MORE_BTN"],
        callback_data=texts[lang_code]["ADD_MORE_BTN"]
    )
    delete_item_button = types.InlineKeyboardButton(
        text=texts[lang_code]["DELETE_ITEM_BTN"],
        callback_data=texts[lang_code]["DELETE_ITEM_BTN"]
    )
    menu = types.InlineKeyboardMarkup(row_width=1)
    menu.add(
        pay_button, add_comment_button, add_more_button,
...
def payment_menu(lang_code: str, order_uuid: str) -> types.InlineKeyboardMarkup:
    ...
    paid_button = types.InlineKeyboardButton(
        text=texts[lang_code]["PAID_BTN"],
        callback_data=f"paid {order_uuid}"
    )
    cancel_button = types.InlineKeyboardButton(
        text=texts[lang_code]["CANCEL_ORDER_BTN"],
        callback_data=f"cancel {order_uuid}"
    )
...
def rest_accept_order_menu(lang_code: str, order_uuid: str) -> types.InlineKeyboardMarkup:
    ...
    accept_button = types.InlineKeyboardButton(
        text=texts[lang_code]["REST_ACCEPT_ORDER_BTN"],
        callback_data=f"accepted {order_uuid}"
    )
